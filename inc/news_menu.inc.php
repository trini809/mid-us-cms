...
// ---------------------------------------------------------------------//
	...
	print '<div id="leftblockmenu">
			<div id="leftnavhead">Newsroom</div>
			<div id="my_menu" class="sdmenu">';
			
	// the query
	$sidenav = db_query ("select * from ".$dbext."child_menu where pid = '91112' and active = '1' order by listorder asc") or db_die();
		...
		// query 3rd level
			$subnav = db_query ("select sid, cid, name from ".$dbext."sub_menu where cid = '".$childitem['cid']."' and active = '1' order by listorder asc") or db_die();
				...
				// sub items
				print '<a href="news.php?pageID='.$subitem[0].'">'.$subitem[2].'</a>';
				}
			print '</div>';
			}
