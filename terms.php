  ...
  <div id="container">
    ...
    <div id="title">
        ...
        <?php 
			...
			$distitle = db_query("select name from ".$dbext."parent_menu where pid = '91118' limit 1") or db_die();
			$pgtitle = fetch_row($distitle);
			echo $pgtitle[0];
			}
		?>
    ...
    <!-- body copy  -->  
     <?php
        ...
        $sql = db_query ("select content from ".$dbext."content where link = '".$pageID."' limit 1") or db_die();
        	...
        	$sql = db_query ("select content from ".$dbext."content where pgid = '65138' limit 1") or db_die();
        	$row = fetch_row($sql);
        	echo stripslashes($row[0]);
        	}
        ?>
